...
st.set_page_config(page_title="Équipe de Recherche", page_icon="◆", layout="wide")
...
st.markdown(
    ...
    .hero-subtitle { color: var(--muted); font-size: 0.98rem; margin-bottom: 1.2rem; max-width: 660px; line-height: 1.5; }
    .hero-rule { border: none; border-top: 1px solid var(--border); margin: 0 0 1.8rem 0; }

    .team-row { display: flex; gap: 0.6rem; margin-bottom: 1.6rem; flex-wrap: wrap; }
    .team-chip {
        display: flex; align-items: center; gap: 0.4rem; font-size: 0.82rem; color: var(--muted);
        border: 1px solid var(--border); border-radius: 999px; padding: 0.35rem 0.8rem; background: var(--surface);
    }
    .team-chip b { color: var(--ink); font-weight: 600; }
    .team-arrow { color: var(--border); align-self: center; }
...
st.markdown('<div class="eyebrow">Agno Team · 2 agents coordonnés</div>', unsafe_allow_html=True)
st.markdown('<div class="hero-title">Équipe de Recherche</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="hero-subtitle">Deux agents collaborent pour produire un rapport fiable : '
    'un Chercheur rassemble les sources via Firecrawl, un Rédacteur recoupe les faits et rédige le rapport final.</div>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="team-row">'
    '<div class="team-chip">🔎 <b>Chercheur</b> — cherche &amp; scrape via Firecrawl</div>'
    '<div class="team-arrow">→</div>'
    '<div class="team-chip">✎ <b>Rédacteur</b> — vérifie &amp; rédige le rapport final</div>'
    "</div>",
    unsafe_allow_html=True,
)
...
with st.form("team_form"):
    ...
    submitted = st.form_submit_button("Lancer l'équipe")

if "result" not in st.session_state:
    st.session_state.result = None

if submitted and query.strip():
    with st.spinner("L'équipe travaille : recherche → vérification → rédaction (peut prendre 1-2 min)..."):
        report, error = run_research(query.strip())
    st.session_state.result = {"query": query.strip(), "report": report, "error": error}

if not st.session_state.result:
    st.markdown(
        '<div class="empty-state">Aucune recherche pour l’instant. '
        "Saisissez un sujet ci-dessus pour lancer l’équipe.</div>",
        unsafe_allow_html=True,
    )
elif st.session_state.result["error"]:
    st.markdown(f'<div class="error-box">Erreur pendant l’exécution de l’équipe : {st.session_state.result["error"]}</div>', unsafe_allow_html=True)
else:
    res = st.session_state.result
    with st.container(border=True, key="report_card"):
        st.markdown('<div class="query-label">Sujet</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="query-text">{res["query"]}</div>', unsafe_allow_html=True)
